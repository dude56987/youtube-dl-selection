...
if [[ -f "$downloadPath/$fileName" ]];then
	...
	# add the header to the error log only once
	header="You can attempt to finish the download using the given URL."
	if ! grep "$header" $downloadPath/youtube-dl_FAILED.log;then
		echo "$header" >> $downloadPath/youtube-dl_FAILED.log
	fi
	# add failed download links to a text file
	echo "Failed to download '$title' at '$selection'" >> $downloadPath/youtube-dl_FAILED.log
